...
public class JoueurIA extends Joueur {
	...
	private void sendResult(Coup coup) { // retour du coup calculé au contrôleur
		controlJeu.publishEvent(coup.getLigne(), coup.getColonne(), this);
	}

	public void calculCoup() { // calcul du coup à jouer en fonction de la force du joueurIA
		p = null;
		switch (force) {
		case DEBUTANT:
	...
	private Coup calculCoupDebutant() { // retourne un coup possible choisi
		...
		if (possibles.size() > 0) {
			...
			coup = new Coup(possibles.get(index).getX(), possibles.get(index)
					.getY(), this.couleur);
		} else {
			coup = null;
		}
