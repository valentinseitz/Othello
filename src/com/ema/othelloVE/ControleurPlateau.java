		...
		if (possible) {
			...
			for (int i = -1; !possible && i <= 1; i++) {
				for (int j = -1; !possible && j <= 1; j++) {
					...
					if (i != 0 || j != 0) {
						// La droite donnée par la cellule testée et la cellule
			...
			if (possible) {
				...
				case BLANC:
					...
					if (jetonATester == jeton) {
						...
						// ait au moins un de couleur opposée entre (distance >
						// 0)
						possible = distance > 0;
						continuer = false;
					} else {
						// Si l'on est sur un jeton de couleur opposée il faut
		...
		while (continuer) {
			...
			case BLANC:
				...
				if (jetonARetourner == jeton) {
					...
					// moins un de couleur opposée entre (distance > 0)
					continuer = false;
				} else {
